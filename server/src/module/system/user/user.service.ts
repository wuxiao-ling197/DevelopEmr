...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    if (createUserDto.password) {
      // const hashPassword = await pbkdf2Sync(createUserDto.password, createUserDto.password, 350000, 64, 'sha512');
      createUserDto.password = hashPwd(createUserDto.password, createUserDto.password, 600000);
      // createUserDto.password = await bcrypt.hashSync(createUserDto.password, bcrypt.genSaltSync(10));
    }

    const res = await this.userRepo.save({ ...createUserDto, loginDate, userType: SYS_USER_TYPE.CUSTOM, createTime });
    // 在createUserDto中postIds定义为number数组，因此可以map
  ...
  async login(user: LoginDto, clientInfo: ClientInfoDto) {
    ...
    if (!(data && verifyPwd(user.password, data.password))) {
  ...
  async register(user: RegisterDto) {
    ...
    user['password'] = hashPwd(user.password, user.password, 600000);
    await this.userRepo.save({ ...user, loginDate, createtime });
    return ResultData.ok();
  }
  ...
  async resetPwd(body: ResetPwdDto) {
    ...
    if (body.password) {
      ...
      const hashPassword = await hashPwd(body.password, body.password, 600000);
  ...
  async updatePwd(user: any, updatePwdDto: UpdatePwdDto) {
    ...
    if (verifyPwd(updatePwdDto.oldPassword, user.user.password)) {
    ...
    const hashPassword = await hashPwd(updatePwdDto.newPassword, updatePwdDto.newPassword, 600000);
    await this.userRepo.update({ userId: user.user.userId }, { password: hashPassword });
    return ResultData.ok();
  }
