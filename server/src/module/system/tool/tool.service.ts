...
export class ToolService {
  ...
  initTableColumn(column: any, table: any) {
    ...
    // 查询字段类型
    if (lowerColumnName.includes('name') || lowerColumnName.includes('login')) {
    ...
    // 类型&性别字段设置下拉框
    else if (lowerColumnName.includes('type') || lowerColumnName.includes('sex') || lowerColumnName.includes('gender')) {
