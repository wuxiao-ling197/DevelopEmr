  ...
  data() {
    return {
      ...
      designerConfig: {
        ...
        generateSFCButton: false,  //是否显示生成SFC按钮（SFC单文件组件，感觉开发的时候比较有用，用户操作的时候没啥用，只用JSON即可）
        toolbarMaxWidth: 300,  //设计器工具按钮栏最大宽度（单位像素）
        toolbarMinWidth: 100,  //设计器工具按钮栏最小宽度（单位像素）
        presetCssCode: '',  //设计器预设CSS样式代码（感觉也是开发的时候比较有用，使用的时候没啥用，只需要宽高占位即可）
        resetFormJson: false,  //是否在设计器初始化时将表单内容重置为空
      },
      bannedWidgets:['table', 'rate', 'switch']
    }
  },
  mounted(){
    console.log(this.$refs.vformDesignerRef);
