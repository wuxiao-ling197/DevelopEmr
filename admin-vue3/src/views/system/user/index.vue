        ...
        >
          ...
          >
            <template #default="scope">
              <span>{{
                scope.row.employee ? scope.row.employee.name : "无"
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column
            label="部门"
            align="center"
            key="employee.department.name.en_US"
            prop="employee.department.name.en_US"
            v-if="columns[3].visible"
            :show-overflow-tooltip="true"
          >
            <template #default="scope">
              <!-- <span>{{ scope.row.employee.department ? scope.row.employee.department.name.zh_CN : '无' }}</span> -->
              <!-- 处理user没有绑定employee或department的问题 zh_CN-->
              <span>{{
                scope.row.employee?.department?.name.en_US || "无"
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template #default="scope">
              <span>{{
                scope.row.employee ? scope.row.employee?.employeeType : "无"
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="手机号码"
            align="center"
            key="employee.workPhone"
            prop="employee.workPhone"
            v-if="columns[5].visible"
            width="120"
          >
            <template #default="scope">
              <span>{{
                scope.row.employee ? scope.row.employee.workPhone : "无"
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template #default="scope">
              <span>{{
                scope.row.employee ? scope.row.notificationType : "无"
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template #default="scope">
              <span>{{
                scope.row.employee ? scope.row.employee.workEmail : "无"
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template #default="scope">
              <span>{{
                scope.row.employee?.gender === "male"
                  ? "男"
                  : scope.row.employee?.gender === "female"
                  ? "女"
                  : "未知"
              }}</span>
            </template>
          ...
          <el-table-column
            label="部门结构"
            align="center"
            key="employee.department.virtual"
            prop="employee.department.virtual"
            v-if="columns[12].visible"
            :show-overflow-tooltip="true"
          >
            <template #default="scope">
              <span>{{
                scope.row.employee?.department?.virtual === true
                  ? "虚拟"
                  : scope.row.employee?.department?.virtual === false
                  ? "实体"
                  : ""
              }}</span>
            </template>
          ...
          >
            <template #default="scope">
              ...
              <el-tooltip content="重置密码" placement="top" v-if="scope.row.id !== 2">
                         <el-button link type="primary" icon="cpu" @click="handleResetPwd(scope.row)" v-hasPermi="['system:user:resetPwd']"></el-button>
                     </el-tooltip>
              <el-tooltip
                content="启用TOTP"
                placement="top"
              ...
              >
                <el-button
                  ...
                  type="primary"
                  icon="Key"
                  @click="handleEnableTotp(scope.row) "
                  v-hasPermi="['system:user:enableTotp']"
                >
                </el-button>
              </el-tooltip>
              <el-tooltip
...
/**启用totp 已经启用的话activateTotp为false,第一次启用的话activateTotp为true*/
function handleEnableTotp(row) {
  const userIds = row.id || ids.value;
  proxy.$modal.confirm('确认启用用户 '+row.login+" 登录时双重验证吗？").then(function () {
     createTotp(userIds).then(res=>{
        if(!res.data.activateTotp){
           setTimeout(()=>{
            proxy.$modal.confirm('用户 '+row.login+' 已启用双重验证，是否需要重置？').then(()=>{
            closeTotp(userIds).then(res=>{
               createTotp(userIds).then(res=>{
